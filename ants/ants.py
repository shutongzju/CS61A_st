...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if issubclass(place.ant.__class__, ContainerAnt) and place.ant.can_contain(self):
                place.ant.contain_ant(self)
            elif issubclass(self.__class__, ContainerAnt) and self.can_contain(place.ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        count = 0
        temp = self.place
        while temp and count < self.min_range:
            temp, count = temp.entrance, count + 1

        while temp and not temp.bees:
            temp, count = temp.entrance, count + 1

        if temp and temp != beehive and self.min_range <= count <= self.max_range:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return not self.contained_ant and not issubclass(other.__class__, ContainerAnt)
        # END Problem 9
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower can survive in water places"""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.ants = QueenAnt.ants[:]
        if not QueenAnt.flag:
            QueenAnt.flag.append(1)
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.flag:
            ...
            while temp:
                if temp.ant:
                    if temp.ant not in self.ants:
                        temp.ant.damage *= 2
                        self.ants.append(temp.ant)
                    if issubclass(temp.ant.__class__, ContainerAnt) and temp.ant.contained_ant:
                        if temp.ant.contained_ant not in self.ants:
                            temp.ant.contained_ant.damage *= 2
                            self.ants.append(temp.ant.contained_ant)
                temp = temp.exit
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while temp != beehive:
            if temp.ant:
                result[temp.ant] = count
                if issubclass(temp.ant.__class__, ContainerAnt) and temp.ant.contained_ant:
                    result[temp.ant.contained_ant] = count
            if temp.bees:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        result = self.damage - self.insects_shot*0.05 - distance*0.2
