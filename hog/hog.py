...
def free_bacon(score):
    ...
    length = len(str(score_cube))
    temp = 0
    while length:
    	temp, length, score_cube = temp + pow(-1, length + 1) * (score_cube % 10), length - 1, score_cube // 10
    scores = 1 + abs(temp)
    return scores
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    scores = pow(3, player_score + opponent_score)
    scores_list = [int(x) for x in str(scores)]
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    bonus = 3 #bonus exist when feral_hogs is True
    score_0 , score_1 = 0, 0
    ...
    while (score0 < goal) and (score1 < goal):
    	...
    	if who == 0:
    		...
    		if feral_hogs and (abs(strategy0(score0, score1) - score_0) == 2):
    			temp = bonus

    		score_0 = take_turn(strategy0(score0, score1), score1, dice)

    		score0 = score0 + temp + score_0

    	
    	else:

    		if feral_hogs and (abs(strategy1(score1, score0) - score_1) == 2):
    			temp = bonus

    		score_1 = take_turn(strategy1(score1, score0), score0, dice)

    		score1 = score1 + temp + score_1
...
def make_averaged(g, num_samples=1000):
    ...
    def average_result(*args):
        ...
        result = 0
        while temp:
            result, temp = result + g(*args), temp - 1
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, temp, score_temp = 1, 1, 0
    max_score = 0
    while temp <= 10:
        score_temp = make_averaged(roll_dice, num_samples)(temp, dice)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if is_swap(score + free_bacon(opponent_score), opponent_score) and (opponent_score - score - free_bacon(opponent_score) > 0):
        return 0
    elif is_swap(score + free_bacon(opponent_score), opponent_score) and (opponent_score - score - free_bacon(opponent_score) < 0):
        return num_rolls
